...
:PROCESS_CMD
    ...
    ENDLOCAL & (
        SET "TOOLSET_NINJA_PATH=%utility_software_folder%"
        SET "PATH=%PATH%"
    )
EXIT /B 0
...
:SHOW_INFO
    ...
    IF %ERRORLEVEL% EQU 0 (
        cecho {olive}[TOOLSET - UTILS - NINJA]{default} INFO: %~1{\n}
    ) ELSE (
        echo [TOOLSET - UTILS - NINJA] INFO: %~1
    )
EXIT /B 0
...
:SHOW_ERROR
    WHERE cecho >nul 2>nul
    IF %ERRORLEVEL% EQU 0 (
        cecho {olive}[TOOLSET - UTILS - NINJA]{red} ERROR: %~1 {default} {\n}
    ) ELSE (
        echo [TOOLSET - UTILS - NINJA] ERROR: %~1
    )
EXIT /B 0
...
:SHOW_HELP
    ...
    ECHO #     TOOLSET_NINJA_PATH    Absolute path where this tool is located. #
    ECHO #                                                                     #
